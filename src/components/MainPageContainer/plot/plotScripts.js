...
export const drawPlot = (attemptsArray) => {
    attemptsArray = JSON.parse(localStorage.getItem("points"))
    if(!attemptsArray){
        attemptsArray = []
    }
...
export const updatePlot = (attemptsArray) => {
    const inputValue = document.querySelector('#r');
    const rValue = inputValue !== null ? inputValue : 3;
    ...
    for (let i = 0; i < pointsArray.length; i++) {
        ...
        drawPoint(point.x, point.y, point.r, point.result, pointsScale);
    }
...
export const drawPlotWithPoints = (attemptsArray) => {
    ...
    drawPoint(lastPoint.x, lastPoint.y, lastPoint.r, lastPoint.result, scaleLastPoint);
    drawRValue(r);
}
...
let drawArea = (r) => {
    ...
    const traingle = (convertX(0)) + ',' + (convertY(0)) + ' ' +
        (convertX(0)) + ',' + (convertY(r)) + ' ' +
        (convertX(-r/2)) + ',' + (convertY(0));
    //console.log('area coordinates ' + area)
    CANVAS.polygon(area).fill(AREA_COLOR)
    CANVAS.polygon(traingle).fill(AREA_COLOR)
}
...
let getCoords = (event, element) => {
    ...
    coordinates.r = document.querySelector("#Rval").value;
    coordinates.x = convertToCoordinatesX((event.clientX - xPosition) * (WIDTH / width))/3 * coordinates.r;
    coordinates.y = convertToCoordinatesY((event.clientY - yPosition) * (HEIGHT / height))/3 * coordinates.r;
